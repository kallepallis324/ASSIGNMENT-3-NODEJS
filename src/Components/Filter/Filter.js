...
class Filter extends Component {
    constructor(props) {
        ...
        this.state = {
            StartDateVal: '',
            EndDateVal: '',
            APILinkForFilteringData: 'https://nodeassignmentt3.herokuapp.com/FilteredData',
            Data: []
        }
    }
    ...
    render() {
        ...
        return (<>
            <div style={{ position: 'relative', width: '100%' }}>
                <form className='formDivision' onSubmit={(e) => {
                    e.preventDefault();
                }}>
                    ...
                    <div className='BtnFilter'>
                        <button onClick={this.btnFilterr} className='FilterButton'>Filter</button>
                    </div>
                </form>
                <button onClick={this.logOff} className='buttonLogOff'>Back To Data Page</button>
            </div>
            <div className='FilteredDataa'>
                <div>
                    ...
                    {this.state.Data.length ? this.state.Data.map((ite, index) => {
                        return (<>
                            <tr>
                                <td className='Border-Table'>{ite.TemperatureLevel}</td>
                                <td className='Border-Table'>{ite.BatteryLevel}</td>
                                <td className='Border-Table'>{ite.Time.split('T')[0] + ' ' + ite.Time.split('T')[1]}</td>
                            </tr>
                        </>)
                    }) : 
                    <tr>
                        <td className='Border-Table' colSpan='3'>Data Not Available</td>
                    </tr>}
                </div>
            </div>
        </>);
...
const mapDispatchToProps = (dispatch) => ({
    UserLoggedIn: (payload, type) => dispatch({
        type: type,
        payload: payload
    }),
})
